...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  -->
  <xsl:template mode="iso19139.mgbcompleto" priority="199"
  ...
  <!--
    ...
    -->
  <xsl:template mode="iso19139.mgbcompleto" match="srv:operatesOn">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:variable name="text">
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="codelistCore">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:copy-of
                select="/root/gui/schemas/*[name(.)='iso19139.mgbcompleto']/codelists/codelist[@name = $qname]/*"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
  ...
  <!-- gmx:Anchor is a substitute of gco:CharacterString and
    ...
    -->
  <xsl:template mode="iso19139.mgbcompleto" match="*[gmx:Anchor]" priority="99">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!--
  placeholder
  <xsl:template mode="iso19139.mgbcompleto" match="TAG">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
      ...
      <td valign="middle" colspan="2">
        ...
        <xsl:if test="/root/gui/config/editor-metadata-relation">
          <div style="float:right;">
            <xsl:call-template name="iso19139.mgbcompleto-relatedResources">
              <xsl:with-param name="edit" select="$edit"/>
            </xsl:call-template>
          </div>
        </xsl:if>
  ...
  <xsl:template name="iso19139.mgbcompleto-complexElementGuiWrapper">
  ...
  <xsl:template name="iso19139.mgbcompletoSimple">
    ...
    <xsl:call-template name="complexElementGui">
      <xsl:with-param name="title" select="/root/gui/strings/metadata"/>
      <xsl:with-param name="content">
        <xsl:call-template name="iso19139.mgbcompletoSimple2">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit" select="$edit"/>
        </xsl:call-template>
      </xsl:with-param>
  ...
  <xsl:template name="iso19139.mgbcompletoSimple2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
      ...
      <xsl:with-param name="content">
        ...
        <xsl:choose>
          ...
          <xsl:when test="string(gmd:protocol/gco:CharacterString)='DB:POSTGIS'
            ...
            <xsl:apply-templates mode="iso19139.mgbcompletoGeoPublisher"
                                 select="gmd:name/gco:CharacterString|gmd:name/gmx:MimeFileType">
              <xsl:with-param name="access" select="'private'"/>
              <xsl:with-param name="id" select="$id"/>
            </xsl:apply-templates>
  ...
  <xsl:template name="iso19139.mgbcompletoBrief">
    <metadata>
      <xsl:choose>
        <xsl:when test="geonet:info/isTemplate='s'">
          <xsl:call-template name="iso19139.mgbcompleto-subtemplate"/>
  ...
  <xsl:template name="iso19139.mgbcompleto-brief">
    ...
    <xsl:for-each select="gmd:contact/*">
      ...
      <xsl:if test="normalize-space($role)!=''">
        <responsibleParty
          role="{geonet:getCodeListValue(/root/gui/schemas, 'iso19139.mgbcompleto', 'gmd:CI_RoleCode', $role)}"
          appliesTo="metadata">
  ...
  <xsl:template mode="briefster" match="*">
    ...
    <xsl:if test="gmd:citation/*/gmd:title">
      <title>
        <xsl:apply-templates mode="iso19139.mgbcompleto-localised" select="gmd:citation/*/gmd:title">
    ...
    <xsl:for-each-group select="gmd:pointOfContact/*"
      ...
      <xsl:variable name="roles"
                    select="string-join(current-group()/gmd:role/*/geonet:getCodeListValue(/root/gui/schemas, 'iso19139.mgbcompleto', 'gmd:CI_RoleCode', @codeListValue), ', ')"/>
  ...
  <xsl:template mode="iso19139.mgbcompletoIsEmpty" match="*|@*|text()">
    <xsl:choose>
      <!-- normal element -->
      <xsl:when test="*">
        <xsl:apply-templates mode="iso19139.mgbcompletoIsEmpty"/>
      </xsl:when>
      <!-- text element -->
  ...
  <xsl:template name="iso19139.mgbcompleto-javascript"/>
