...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template name="view-with-header-iso19139.mgbcompleto">
    <xsl:param name="tabs"/>
    ...
    <xsl:call-template name="md-content">
      ...
      <xsl:with-param name="iso19139.mgbcompleto-relatedResources">
        <xsl:apply-templates mode="iso19139.mgbcompleto-relatedResources"
                             select="."
        />
      </xsl:with-param>
  ...
  <!-- ===================================================================== -->
  <!-- iso19139.mgbcompleto-simple -->
  <xsl:template name="metadata-iso19139.mgbcompletoview-simple" match="metadata-iso19139.mgbcompletoview-simple">
    <!--<xsl:apply-templates mode="iso19139.mgbcompleto-simple" select="*"/>-->

    <xsl:call-template name="view-with-header-iso19139.mgbcompleto">
      <xsl:with-param name="tabs">
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title"
                          select="/root/gui/schemas/iso19139.mgbcompleto/strings/understandResource"/>
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/contactInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="block"
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/mdContactInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="block"
                                 select="gmd:contact"/>
          </xsl:with-param>
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/techInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="block"
                                 select="
  ...
  <xsl:template mode="block"
    ...
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/refDate"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19139.mgbcompleto-simple"
  ...
  <xsl:template mode="block" match="gmd:identificationInfo/*/gmd:extent/*/gmd:temporalElement"
                priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/temporalRef"/>
      <xsl:with-param name="helpLink">
        <xsl:call-template name="getHelpLink">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="name" select="name(.)"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19139.mgbcompleto-simple" select="*/gmd:extent/*/gml:beginPosition
  ...
  <xsl:template mode="block" match="gmd:identificationInfo/*/gmd:resourceConstraints[1]"
                priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19139.mgbcompleto/strings/constraintInfo"/>
      <xsl:with-param name="helpLink">
        <xsl:call-template name="getHelpLink">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="name" select="name(.)"/>
  ...
  <xsl:template mode="block" match="gmd:contact|gmd:pointOfContact" priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:value-of
          select="geonet:getCodeListValue(/root/gui/schemas, 'iso19139.mgbcompleto', 'gmd:CI_RoleCode', */gmd:role/gmd:CI_RoleCode/@codeListValue)"
        />
      </xsl:with-param>
      <xsl:with-param name="helpLink">
        <xsl:call-template name="getHelpLink">
    ...
    <xsl:call-template name="simpleElementSimpleGUI">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:variable name="label">
          <xsl:choose>
            <xsl:when
              test="normalize-space($schemaLabel) = '' and starts-with($schema, 'iso19139.mgbcompleto.')">
              <!-- Check iso19139.mgbcompleto label -->
              <xsl:value-of
                select="/root/gui/schemas/*[name(.)='iso19139.mgbcompleto']/codelists/codelist[@name = $name]/entry[code = $choiceValue]/label"/>
            </xsl:when>
  ...
  <xsl:template mode="block"
                ...
                priority="99">
    <xsl:apply-templates mode="iso19139.mgbcompleto" select="gmd:EX_GeographicBoundingBox">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit" select="false()"/>
    </xsl:apply-templates>
  </xsl:template>
...
-->
  <xsl:template mode="iso19139.mgbcompleto-relatedResources"
                match="*">
    <table class="related">
      <tbody>
        <tr
          ...
          <tr>
            <td class="main">
              ...
              <span class="{$protocolIcon} icon">
                <xsl:value-of
                  select="/root/gui/schemas/iso19139.mgbcompleto/labels/element[@name = 'gmd:protocol']/helper/option[@value=normalize-space(current-grouping-key())]"/>
              </span>
            </td>
            <td>
              <ul>
  ...
  <!-- Hide them -->
  <xsl:template mode="iso19139.mgbcompleto-simple" match="
    geonet:*|*[@gco:nilReason='missing']|@gco:isoType" priority="99"/>
  <!-- Don't display -->

  <!-- these elements should be boxed -->
  <xsl:template mode="iso19139.mgbcompleto-simple"
    ...
    <xsl:call-template name="simpleElement">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:if
          ...
          (
          <xsl:apply-templates mode="iso19139.mgbcompletoGetAttributeText"
  ...
  <!-- TODO : PT_FreeText -->
  <xsl:template mode="iso19139.mgbcompleto-simple" match="*[gco:CharacterString]" priority="2">
  ...
  <xsl:template mode="iso19139.mgbcompleto-simple" match="gml:endPosition|gml:beginPosition|gml:timePosition"
                priority="2">
    <xsl:call-template name="simpleElement">
      <xsl:with-param name="id" select="generate-id(.)"/>
      <xsl:with-param name="title">
  ...
  <xsl:template mode="iso19139.mgbcompleto-simple" match="gmd:*[*/@codeList]|srv:*[*/@codeList]">
