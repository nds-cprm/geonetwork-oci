...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" priority="2000"
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" match="gmd:MD_Keywords" priority="2000">
      ...
      <xsl:when test="$thesaurusConfig">
        ...
        <xsl:variable name="transformations"
                      ...
                      select="if ($thesaurusConfig/@transformations != '')
                              ...
                              else 'to-iso19139.mgbcompleto-keyword,to-iso19139.mgbcompleto-keyword-with-anchor,to-iso19139.mgbcompleto-keyword-as-xlink'"/>

        <!-- Get current transformation mode based on XML fragment analysis -->
        <xsl:variable name="transformation"
                      select="if (parent::node()/@xlink:href) then 'to-iso19139.mgbcompleto-keyword-as-xlink'
          else if (count(gmd:keyword/gmx:Anchor) > 0)
          then 'to-iso19139.mgbcompleto-keyword-with-anchor'
  ...
  <xsl:template name="iso19139.mgbcompleto-addAllThesaurus">
    <xsl:param name="ref"/>
    <xsl:if test="java:getSettingValue('system/metadata/allThesaurus') = 'true'">
      ...
      <div
              ...
              data-keywords=""
              data-transformations="{$transformations}"
              data-current-transformation="to-iso19139.mgbcompleto-keyword"
