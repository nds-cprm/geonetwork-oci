...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
  <xsl:template mode="mode-iso19139.mgbcompleto" match="*[gmd:CI_Date]" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="overrideLabel" select="''" required="no"/>

    <xsl:apply-templates mode="mode-iso19139.mgbcompleto" select="*/gmd:*">
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" match="gmd:dateType" priority="4000"/>
