...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
                ...
                xmlns:gn-fn-iso19139.mgbcompleto="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139.mgbcompleto"
                version="2.0"
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto"
                match="gn:*|@gn:*|@*"
                priority="1000"/>
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" match="gn:child" priority="2000">
    ...
    <xsl:if test="$name = 'gmd:descriptiveKeywords' and count(../gmd:descriptiveKeywords) = 0">
      <xsl:call-template name="iso19139.mgbcompleto-addAllThesaurus">
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" match="gmd:*|gmx:*|gml:*|gml320:*|srv:*|gts:*">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:apply-templates mode="mode-iso19139.mgbcompleto" select="*|@*">
  ...
  -->
  <xsl:template mode="mode-iso19139.mgbcompleto" priority="200"
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" priority="200"
    ...
    <xsl:variable name="excluded"
                  select="gn-fn-iso19139.mgbcompleto:isNotMultilingualField(., $editorConfig)"/>
  ...
  <!-- Display UUIDREF attribute with the parent element name
   as read only. The associated resource panel is used to edit
    those values. -->
  <xsl:template mode="mode-iso19139.mgbcompleto" match="@uuidref" priority="2000">
    <xsl:variable name="xpath" select="gn-fn-metadata:getXPath(.)"/>
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" match="gco:ScopedName|gco:LocalName">
  ...
  -->
  <xsl:template mode="mode-iso19139.mgbcompleto" priority="200" match="*[*/@codeList]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="cls" select="local-name()"/>
      <xsl:with-param name="xpath" select="$xpath"/>
      <xsl:with-param name="type" select="gn-fn-iso19139.mgbcompleto:getCodeListType(name())"/>
  ...
  -->
  <xsl:template mode="mode-iso19139.mgbcompleto" match="*[gn:element/gn:text]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$codelists" required="no"/>
  ...
  <xsl:template mode="mode-iso19139.mgbcompleto" priority="100"
