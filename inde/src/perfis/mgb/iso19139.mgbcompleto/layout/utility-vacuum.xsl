...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  * All elements having no child text (ie. normalize-space return '') and
  ...
  The main function call the vacuum-iso19139.mgbcompleto mode.
  -->
  <xsl:function name="gn-fn-iso19139.mgbcompleto:vacuum" as="node()">
    <xsl:param name="metadata" as="node()"/>
    <xsl:for-each select="$metadata/*">
      <xsl:apply-templates mode="vacuum-iso19139.mgbcompleto"
                           select="."/>
    </xsl:for-each>
  </xsl:function>


  <xsl:function name="gn-fn-iso19139.mgbcompleto:isElementOrChildEmpty"
                as="xs:boolean">
  ...
  <xsl:template mode="vacuum-iso19139.mgbcompleto"
                match="@*|node()">
    <xsl:variable name="isElementEmpty"
                  select="gn-fn-iso19139.mgbcompleto:isElementOrChildEmpty(.)"/>
