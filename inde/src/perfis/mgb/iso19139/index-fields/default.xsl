...
<xsl:stylesheet xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template match="*" mode="metadata">
    ...
    <xsl:for-each select="gmd:identificationInfo//gmd:MD_DataIdentification|
      ...
      <xsl:for-each select="gmd:pointOfContact[1]/*/gmd:role/(*/@codeListValue|gco:CharacterString/text())">
      ...
      <xsl:for-each select=".//gmd:EX_Extent">
        <xsl:apply-templates select="gmd:geographicElement/gmd:EX_GeographicBoundingBox|gmd:EX_GeographicBoundingBox"
                             mode="latLon"/>
      ...
      <xsl:for-each select=".//gmd:spatialResolution[.//gmd:denominator/gco:Integer/text() != ''][1]/gmd:MD_Resolution">
        <xsl:for-each
    ...
    <xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution">
      ...
      <xsl:for-each select="gmd:transferOptions/gmd:MD_DigitalTransferOptions|.">
        ...
        <xsl:for-each select=".//gmd:onLine/gmd:CI_OnlineResource[gmd:linkage/gmd:URL!='' or gmd:linkage/gco:CharacterString!='']">
          <xsl:variable name="download_check">
            <xsl:text>&amp;fname=&amp;access</xsl:text>
          </xsl:variable>
          <xsl:variable name="linkage" select="gmd:linkage/gmd:URL|gmd:linkage/gco:CharacterString"/>
  ...
  <xsl:template mode="index-contact" match="gmd:CI_ResponsibleParty|*[@gco:isoType = 'gmd:CI_ResponsibleParty']">
    ...
    <xsl:variable name="role" select="gmd:role/(*/@codeListValue|gco:CharacterString/text())"/>
    ...
    <xsl:variable name="website" select=".//gmd:onlineResource/*/gmd:linkage/(gmd:URL|gco:CharacterString)"/>
    ...
    <xsl:variable name="usefulContent"
                  select="normalize-space(concat($orgName, $logo, string-join($email, ''), 
                            $individualName, $positionName, $address, string-join($phone, ''),
                            $website))"/>
    <xsl:if test="$usefulContent != ''">
    <!-- Nao mudei a identacao para reduzir as diferencas apenas ao que interessa -->
