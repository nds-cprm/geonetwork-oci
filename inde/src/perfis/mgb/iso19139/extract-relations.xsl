...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template mode="relation"
    ...
    <xsl:if test="count(*/descendant::*[name(.) = 'gmd:onLine']/*[gmd:linkage/gmd:URL!='' or gmd:linkage/gco:CharacterString!='']) > 0">
      <onlines>
        <xsl:for-each select="*/descendant::*[name(.) = 'gmd:onLine']/*[gmd:linkage/gmd:URL!='' or gmd:linkage/gco:CharacterString!='']">
          <item>
            <xsl:variable name="langCode">
              <xsl:value-of select="concat('#', upper-case(util:twoCharLangCode($lang, 'EN')))"/>
            </xsl:variable>
            <xsl:variable name="url" select="gmd:linkage/gmd:URL|gmd:linkage/gco:CharacterString"/>
            <id>
