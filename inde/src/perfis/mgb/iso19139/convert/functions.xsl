...
<xsl:stylesheet xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco"
  ...
  <xsl:template name="langId19139">
    ...
    <xsl:variable name="tmp">
      <!-- IBGE 2020: forca idioma a ser sempre portugues, mesmo com MDs ruins
      <xsl:choose>
  ...
  <xsl:template name="defaultAbstract">
    ...
    <xsl:variable name="abstract"
                  select="$identification/gmd:abstract|//gmd:abstract"/>
    <xsl:variable name="docLangAbstract"
                  select="$abstract//gmd:LocalisedCharacterString[@locale=$poundLangId]"/>
    <xsl:variable name="charStringAbstract"
                  select="$abstract/gco:CharacterString"/>
    <xsl:variable name="locStringAbstracts"
                  select="$abstract//gmd:LocalisedCharacterString[. != '']"/>
    <xsl:choose>
