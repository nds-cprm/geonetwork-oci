...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="../../iso19139.mgbsumarizado/convert/functions.xsl"/>
  ...
  -->
  <xsl:template name="iso19139.mgbsumarizado-localised" mode="iso19139.mgbsumarizado-localised" match="*[gco:CharacterString or gmd:PT_FreeText]">
  ...
  <xsl:template mode="iso19139.mgbsumarizado-localised" match="*[not(gco:CharacterString or gmd:PT_FreeText)]">
    <xsl:param name="langId"/>
    <xsl:value-of select="*[1]"/>
  </xsl:template>

  <!-- Check if the element has hidden subelements -->
  <xsl:template mode="iso19139.mgbsumarizado-localised"
  ...
  <xsl:template name="iso19139.mgbsumarizado-getLangId">
  ...
  <!-- Get lang #id in metadata PT_Locale section,  deprecated: if not return the 2 first letters
      ...
      -->
  <xsl:template name="iso19139.mgbsumarizado-getLangIdFromMetadata">
    <xsl:param name="md"/>
    <xsl:param name="lang"/>
  ...
  <xsl:template name="iso19139.mgbsumarizado-getLangCode">
    <xsl:param name="md"/>
    <xsl:param name="langId"/>
  ...
  <!-- Display related metadata records. Related resource are only iso19139.mgbsumarizado/119 or iso19110 metadate records for now.
    ...
    -->
  <xsl:template name="iso19139.mgbsumarizado-relatedResources">
    <xsl:param name="edit"/>

    <xsl:variable name="metadata"
                  select="/root/gmd:MD_Metadata|/root/*[@gco:isoType='gmd:MD_Metadata']"/>
    <xsl:if test="starts-with(geonet:info/schema, 'iso19139.mgbsumarizado') or geonet:info/schema = 'iso19110'">
        ...
        <div class="relatedElements">
          ...
          <xsl:if
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <!-- update child option only for iso19139.mgbsumarizado schema based metadata and admin user -->
  ...
  <xsl:template name="iso19139.mgbsumarizado-getServiceURL">
