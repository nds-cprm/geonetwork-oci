...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
  ...
  <sch:title xmlns="http://www.w3.org/2001/XMLSchema" xml:lang="pt">Regras ISO</sch:title>
  ...
  <sch:pattern id="rule.mdb.defaultlocale">
    ...
    <sch:rule
      ...
      <sch:let name="hasNilReason" value="string(lan:PT_Locale/lan:characterEncoding/@gco:nilReason) != ''"/>

      <sch:assert test="$hasEncoding or $hasNilReason"
                  diagnostics="rule.mdb.defaultlocale-failure-en                      rule.mdb.defaultlocale-failure-fr"/>


      <sch:report test="$hasEncoding or $hasNilReason"
  ...
  <sch:pattern id="rule.mdb.scope-name">
    ...
    <sch:rule
      ...
      <sch:let name="hasScopeCodeName"
               value="normalize-space($scopeCodeName) = ''"/> <!-- EMX -->
  ...
  <sch:pattern id="rule.mdb.create-date">
    ...
    <sch:rule context="mdb:MD_Metadata">
      ...
      <!-- EMX: It can be a DateTime or a Date -->

      <sch:let name="hasAtLeastOneCreationDateT"
               value="count(./mdb:dateInfo/cit:CI_Date[
                normalize-space(cit:date/gco:DateTime) != '' and cit:dateType/cit:CI_DateTypeCode/@codeListValue = 'creation']) &gt; 0"/>
      
      <sch:let name="hasAtLeastOneCreationDate"
               value="count(./mdb:dateInfo/cit:CI_Date[
                normalize-space(cit:date/gco:Date) != '' and cit:dateType/cit:CI_DateTypeCode/@codeListValue = 'creation']) &gt; 0"/>

      <sch:assert test="$hasAtLeastOneCreationDateT or $hasAtLeastOneCreationDate"
                  diagnostics="rule.mdb.create-date-failure-en                      rule.mdb.create-date-failure-fr"/>

      <sch:report test="$hasAtLeastOneCreationDateT or $hasAtLeastOneCreationDate"
