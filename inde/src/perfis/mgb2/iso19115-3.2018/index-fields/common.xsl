...
<xsl:stylesheet version="2.0"
  ...
  -->
  <xsl:variable name="defaultLang">por</xsl:variable>
  ...
  <xsl:template name="CommonFieldsFactory">
    ...
    <xsl:for-each select="$metadata/mdb:identificationInfo/*">
      ...
      <xsl:variable name="listOfKeywords">{
        ...
        <xsl:if test="count(//mri:MD_Keywords[not(mri:thesaurusName) or mri:thesaurusName/*/cit:title/*/text() = '']) > 0">
   ...
   <!-- TODO: Multilingual support -->
    ...
    <!-- IBGE -->
    <xsl:for-each select="$metadata/mdb:resourceLineage/*/mrl:source">
      <xsl:variable name="title"
                    select="normalize-space(*/mrl:sourceCitation/cit:CI_Citation/cit:title/*/text())" />
      <xsl:if test="$title != ''">
        <xsl:variable name="details">
          {
            "title": "<xsl:value-of select="gn-fn-index:json-escape($title)"/>",
            "date": "<xsl:value-of select="gn-fn-index:json-escape(string-join(*/mrl:sourceCitation/cit:CI_Citation/cit:date//(gco:Date|gco:DateTime)/text(), ','))"/>",
            "onlineResource": "<xsl:value-of select="gn-fn-index:json-escape(string-join(*/mrl:sourceCitation/cit:CI_Citation/cit:onlineResource/*/cit:linkage/*/text(), ','))"/>",
          }
        </xsl:variable>
        <!-- Nao extraido: sourceStep -->
        <Field name="_lineageSource" string="{normalize-space($details)}" store="true" index="false"/>
      </xsl:if>
    </xsl:for-each>
    
    <xsl:for-each select="$metadata/mdb:resourceLineage/*/mrl:processStep">
      <xsl:variable name="description"
                    select="normalize-space(*/mrl:description/*/text())" />
      <xsl:if test="$description != ''">
        <xsl:variable name="details">
          {
            "description": "<xsl:value-of select="gn-fn-index:json-escape($description)"/>",
            "dateTime": "<xsl:value-of select="gn-fn-index:json-escape(*/mrl:stepDateTime/gml:TimeInstant/gml:timePosition/text())"/>",
            "processor": "<xsl:value-of select="gn-fn-index:json-escape(string-join(*/mrl:processor/cit:CI_Responsibility/cit:party/*/cit:name/*/text(), ','))"/>",
            "reference": "<xsl:value-of select="gn-fn-index:json-escape(string-join(*/mrl:reference/cit:CI_Citation/cit:title/*/text(), ','))"/>",
          }
        </xsl:variable>
        <!-- Nao extraido: source, mais detalhes do processor -->
        <Field name="_lineageProcessStep" string="{normalize-space($details)}" store="true" index="false"/>
      </xsl:if>
    </xsl:for-each>
